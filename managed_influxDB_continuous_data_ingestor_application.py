...
def createWriteRecordCommonAttributes(dimensions):
    return_dimensions = dict({})
    for dimName in dimensions._fields:
        #return_dimensions.update(dimName,getattr(dimensions, dimName))
        return_dimensions.update(createdimRecord(dimName,getattr(dimensions, dimName)))
    return return_dimensions
    #return { "Dimensions": { dimName: getattr(dimensions, dimName)} for dimName in dimensions._fields }
...
def createRandomMetrics(hostId, timestamp, timeUnit, args):
    ...
    records.update(createRecord(measureCpuUser, cpuUser))
    ...
    for measure in otherCpuMeasures:
        ...
        records.update(createRecord(measure, value))

    cpuIdle = round(max([100 - cpuUser - totalOtherUsage, 0]),2)
    records.update(createRecord(measureCpuIdle, cpuIdle))
    ...
    records.update(createRecord(measureMemoryFree, "{}".format(memFree)))
    records.update(createRecord(measureMemoryUsed, "{}".format(memUsed)))
    records.update(createRecord(measureMemoryCached,"{}".format(4000.0 * random.random())))
    # Disc metrics 
    discUpDown = random.gauss(5.0, 5.0)
    records.update(createRecord(measureDiskUsed,"{}".format(hostId * 30.0 + discUpDown)))
    records.update(createRecord(measureDiskFree,"{}".format(15.0 - discUpDown)))
    records.update(createRecord(measureFileDescriptors, "{}".format(random.gauss(2200.0, 500.0))))
    records.update(createRecord(measureDiskIoReads, "{}".format(25000.0 * random.random())))
    records.update(createRecord(meausreDiskIoWrites, "{}".format(25000.0 * random.random())))
    # Network metrics
    records.update(createRecord(measureLatencyPerRead, "{}".format(random.gauss(8.0, 4.0))))
    records.update(createRecord(measureLatencyPerWrite, "{}".format(random.gauss(12.0, 8.0))))
    records.update(createRecord(measureNetworkBytesIn, "{}".format(1000.0 * random.random())))
    records.update(createRecord(measureNetworkBytesOut, "{}".format(5000.0 * random.random())))
...
def createRandomEvent(hostId, timestamp, timeUnit, args):
    ...
    records.update(createRecord(measureTaskCompleted, random.randint(0, 500)))
    records.update(createRecord(measureTaskEndState, np.random.choice(measureValuesForTaskEndState, p=selectionProbabilities)))
...
def createRecord(measureName, measureValue):
    if measureName == "task_end_state":
        print("string task")
        return {
        measureName: str(measureValue)
    }
    else:
        return {
        measureName: float(measureValue)
    }
def createdimRecord(dimName,dimValue):
    return {
        dimName: dimValue
    }    
   
    
def createCombinedRecord(dimension, metrics):
    return { "measurement": "devops_metrics",
            "tags":dimension,
            "fields":metrics
    }
        ...
        while True:
            ...
            try:
                if not (args.dryRun):
                    ...
                    combined_records = createCombinedRecord(commonAttributes, records)
                    print ("Combined records",combined_records)
                    writeResult = writeRecords(self.client, args.bucket, args.org, combined_records)
...
def createWriteClient(url, token, org):
...
def writeRecords(client,bucket,org,records):

    # Write script Synchronous
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog = 'Timestream for influxDB SampleContinuousDataIngestorApplication', description='Execute an application generating and ingesting time series data.')


    parser.add_argument('--url', '-url', dest="url", action = "store", required = True, \
                        help = "The managed influxDB access url e.g. https://influxhost:8086 .")
    parser.add_argument('--token', '-t', dest="token", action = "store", required = True, \
                        help = "The token for access managed influxDB - must be already created.")

    parser.add_argument('--bucket', '-b', dest="bucket",action = "store", required = True, \
                        help = "The bucket of managed influxDB - must be already created.")
    
    parser.add_argument('--org', '-o', dest="org",action = "store", required = True, \
                        help = "The org of managed influxDB - must be already created.")
    ...
    try:
        tsClient = any
        if not (args.dryRun):
            tsClient = createWriteClient(args.url, args.token,  args.org)
    except Exception as e:
        print(e)
        sys.exit(0)
